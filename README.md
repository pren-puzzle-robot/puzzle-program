...
| `solver` | `algorithm` | `fast` | Algorithm to use for solving the puzzle. Supported values: `fast`, `greedy`, `brute_force`, `edge_walk`. `brute_force` combines detected outer edges into a closed rectangular boundary and scores layouts whose short-to-long side ratio is close to `1:sqrt(2)`. `edge_walk` walks a cursor around a derived A5-style frame and recursively places pieces by high-ranked straight outer edges; it uses no piece-to-piece gap, allows only small piece overlap, tries progressively larger derived frames instead of relying on overlap, keeps the lowest-overlap layout it finds, and reports no solution if every frame-walk placement exceeds the overlap cap. |
